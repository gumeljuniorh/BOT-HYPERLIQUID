...
export class HyperliquidValidationRunner {
    ...
    try {
      ...
      if (snapshot) {
         ...
         if (exchangeSize === snapshotSize) {
           ...
           botState.phase = (savedPhase && savedPhase !== "VALIDATION_FAILED") ? (savedPhase as any) : "PHASE_2_ADAPTIVE_EXECUTION";
  ...
  private async pollForPosition(symbol: string, targetSize: number, maxRetries = 15): Promise<boolean> {
    const { syncAccountState } = await import("./bot.js");
    for (let i = 0; i < maxRetries; i++) {
        ...
        if (targetSize === 0) {
           ...
           if (szi >= targetSize * 0.99) { // Allow for slight rounding
               return true;
           }
        }
